...
/**
 * Default implementation for order item service.
 *
 * @package MShop
 * @subpackage Order
 */
...
{
	/**
	 * Initializes the order service item
	 *
	...
	{
		return $this->set( 'order.service.siteid', (string) $value );
	}


	/**
	 * Returns the order ID of the order service if available.
	 *
	 * @return string|null Base ID of the item.
	 */
	public function getOrderId()
	{
		return $this->get( 'order.service.orderid' );
	}


	/**
	 * Sets the order service order ID of the order service item.
	 *
	 * @param string $value Order service order ID
	 * @return \Aimeos\MShop\Order\Item\Service\Iface Order service item for chaining method calls
	 */
	public function setOrderId( $value )
	{
		return $this->set( 'order.service.orderid', (string) $value );
	}
	...
	{
		return (string) $this->get( 'order.service.serviceid', '' );
	}
	...
	{
		return $this->set( 'order.service.serviceid', (string) $servid );
	}
	...
	{
		return (string) $this->get( 'order.service.code', '' );
	}
	...
	{
		return (string) $this->get( 'order.service.name', '' );
	}
	...
	{
		return $this->set( 'order.service.name', (string) $name );
	}
	...
	{
		return (string) $this->get( 'order.service.type', '' );
	}
	...
	{
		return (string) $this->get( 'order.service.mediaurl', '' );
	}
	...
	{
		return $this->set( 'order.service.mediaurl', (string) $value );
	}
	...
	public function setPosition( $value )
	{
		...
		return $this->set( 'order.service.position', ( $value !== null ? (int) $value : null ) );
	}
	...
	public function fromArray( array &$list, $private = false )
	{
		...
		foreach( $list as $key => $value )
		{
			switch( $key )
			{
				...
				case 'order.service.orderid': !$private ?: $item = $item->setOrderId( $value ); break;
	...
	public function toArray( $private = false )
	{
		...
		if( $private === true )
		{
			$list['order.service.orderid'] = $this->getOrderId();
