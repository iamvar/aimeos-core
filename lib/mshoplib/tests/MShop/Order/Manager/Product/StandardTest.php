...
class StandardTest extends \PHPUnit\Framework\TestCase
{
	...
	public function testAggregate()
	{
		...
		$search->setConditions( $search->compare( '==', 'order.product.editor', 'core:lib/mshoplib' ) );
		$result = $this->object->aggregate( $search, 'order.product.stocktype' );
	...
	public function testAggregateAvg()
	{
		$search = $this->object->createSearch();
		$search->setConditions( $search->compare( '==', 'order.product.editor', 'core:lib/mshoplib' ) );
		$result = $this->object->aggregate( $search, 'order.product.type', 'order.product.price', 'avg' );
	...
	public function testAggregateSum()
	{
		$search = $this->object->createSearch();
		$search->setConditions( $search->compare( '==', 'order.product.editor', 'core:lib/mshoplib' ) );
		$result = $this->object->aggregate( $search, 'order.product.type', 'order.product.quantity', 'sum' );
	...
	{
		$this->assertInstanceOf( \Aimeos\MShop\Order\Item\Product\Iface::class, $this->object->createItem() );
	}
	...
	public function testSearchItem()
	{
		...
		$expr[] = $search->compare( '==', 'order.product.stocktype', 'unit_type1' );
		...
		$expr[] = $search->compare( '==', 'order.product.tax', '0.00' );
		...
		$conditions = array(
			$search->compare( '==', 'order.product.suppliercode', 'unitsupplier' ),
	...
	public function testSaveUpdateDeleteItem()
	{
		$search = $this->object->createSearch();
		$conditions = array(
			$search->compare( '==', 'order.product.price', '0.00' ),
			$search->compare( '==', 'order.product.editor', $this->editor )
		);
		$search->setConditions( $search->combine( '&&', $conditions ) );
		...
		$this->assertEquals( $item->getOrderId(), $itemSaved->getOrderId() );
		...
		$this->assertEquals( $itemExp->getOrderId(), $itemUpd->getOrderId() );
