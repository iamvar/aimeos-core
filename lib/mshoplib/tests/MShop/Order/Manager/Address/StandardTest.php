...
class StandardTest extends \PHPUnit\Framework\TestCase
{
	...
	protected function setUp()
	{
		...
		$this->object = $orderManager->getSubManager( 'address' );
	}
	...
	public function testAggregate()
	{
		...
		$search->setConditions( $search->compare( '==', 'order.address.editor', 'core:lib/mshoplib' ) );
		$result = $this->object->aggregate( $search, 'order.address.salutation' );
	...
	public function testGetItem()
	{
		...
		if( ( $item = reset( $items ) ) === false ) {
			throw new \RuntimeException( sprintf( 'No order address item found for type "%1$s".', $type ) );
		}
